...
public class FileUtils {
	...
	public static void readGraph(String graphFile, Node[] nodes)
		...
		while (null != (line = br.readLine())) {
			...
			if (nodes[idx1 - 1] != null) {
				nodes[idx1 - 1] = new Node(idx1, 1);
			} else {
				nodes[idx1 - 1].addFrequnce();
			}
			if (nodes[idx2 - 1] != null) {
				nodes[idx2 - 1] = new Node(idx2, 1);
			} else {
				nodes[idx2 - 1].addFrequnce();
			}
		}
		br.close();
	}

	public static void readCluster(String clusterFile, Node[] nodes,
			List<Set<Node>> clusters) throws IOException {
		...
		while (null != (line = br.readLine())) {
			String[] items = line.split("\\s+");
			clusters.set(i, new HashSet<Node>());
			for (String item : items) {
				int idx = Integer.parseInt(item);
				nodes[idx-1].addCluster(i);
				clusters.get(i).add(nodes[idx-1]);
			}
			i++;
		}
		br.close();
